...
#change column names to just the identifier ls *.out | parallel --dryrun -j30 kaiju2table -t ~/kaijudb/nodes.dmp -n ~/kaijudb/names.dmp -e -r species -l domain,superkingdom,phylum,class,order,family,genus,species -o {.}.tsv {}find and replace reads with nothing?
...
df_viruses=df_taxa_reads[df_taxa_reads['domain']=='Viruses']
df_viruses.to_csv('OTU_viruses.csv')
